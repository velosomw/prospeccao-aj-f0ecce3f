  ...
  const doc = new Document({
    ...
    sections: [{
      ...
      children: [
        ...
        new Paragraph({ heading: HeadingLevel.HEADING_1, children: [new TextRun("Ofício de Pendências — RMA")] }),
        new Paragraph({ children: [
          new TextRun({ text: "RMA: ", bold: true }), new TextRun(meta.rmaCode),
        ...
        new Paragraph({ children: [new TextRun(
          "O presente ofício formaliza a relação de pendências, inconsistências e documentos faltantes identificados pelo Administrador Judicial durante a análise do RMA em referência, nos termos da Recomendação CNJ 72/2020."
        )] }),
        ...
        new Paragraph({ children: [new TextRun(
          "Solicita-se à Recuperanda o saneamento das pendências acima no prazo de 15 (quinze) dias corridos, mediante upload na plataforma BEx-RMA e/ou resposta formal por escrito. Pendências não sanadas serão consignadas no Relatório Mensal de Atividades (RMA) submetido ao Juízo."
        )] }),
        new Paragraph({ children: [new TextRun("")] }),
        new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: meta.emitidoPor || "Administração Judicial — BEx-RMA IA", bold: true })] }),
      ],
    }],
  });

  const blob = await Packer.toBlob(doc);
  saveAs(blob, `Oficio_Pendencias_${meta.rmaCode}.docx`);
}
...
export function generateOficioPdf(pendencias: PendenciaItem[], meta: OficioMeta) {
  ...
  pdf.text("Ofício de Pendências — RMA", margin, y);

  y += 8;
  pdf.setFontSize(10); pdf.setTextColor(30); pdf.setFont("helvetica", "normal");
  pdf.text(`RMA: ${meta.rmaCode}    Mês de referência: ${meta.mesReferencia || "—"}`, margin, y);
  ...
  pdf.setFont("helvetica", "bold"); pdf.setTextColor(11, 58, 130);
  pdf.text(meta.emitidoPor || "Administração Judicial — BEx-RMA IA", pageW / 2, y, { align: "center" });

  pdf.save(`Oficio_Pendencias_${meta.rmaCode}.pdf`);
}
