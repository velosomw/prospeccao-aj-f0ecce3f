...
export function useRmaDocument(prospeccaoId: string, tipo: "parecer_tecnico" | "prospeccao_mensal", titulo: string) {
  ...
  const updateContent = async (sectionId: string, conteudo: string, status?: SectionStatus) => {
    ...
    const { error } = await supabase.from("prospeccao_document_sections" as any).update(patch).eq("id", sectionId);
    if (error) {
      toast({ title: "Erro ao salvar", description: error.message, variant: "destructive" });
      return;
    }
    // versão manual
    await supabase.from("prospeccao_document_section_versions" as any).insert({
  ...
  const assignTo = async (sectionId: string, target: "usuario" | "coordenador") => {
    await supabase.from("prospeccao_document_sections" as any).update({ assigned_to: target }).eq("id", sectionId);
    await reloadSection(sectionId);
  };
  ...
  const addComment = async (sectionId: string, text: string) => {
    ...
    const { data } = await supabase.from("prospeccao_document_section_comments" as any).insert(insert).select().single();
