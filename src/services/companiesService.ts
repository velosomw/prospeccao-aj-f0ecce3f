...
export async function createCompany(
  ...
  if (topics.length > 0) {
    ...
    const { error: tErr } = await supabase.from("company_rma_topics").insert(rows);
    if (tErr) throw tErr;
  }
...
export interface CompaniesPageOpts {
  ...
  search?: string;        // filtra por name/cnpj/rma_id
...
export async function listCompaniesPage(
  ...
  if (opts.search && opts.search.trim()) {
    ...
    q = q.or(`name.ilike.%${s}%,cnpj.ilike.%${s}%,rma_id.ilike.%${s}%`);
  }
 ...
 *        'released' (rma_release_assignments.released_to_user_id = userId).
 */
 ...
 * via rma_release_assignments, no backend.
 */
...
/** Lista as empresas (RMAs) atribuídas ao consultor logado */
...
/** Atualiza dados de cadastro da empresa/RMA. */
...
/** Exclui uma empresa/RMA. */
export async function deleteCompany(companyId: string): Promise<void> {
  const { error } = await supabase.from("companies").delete().eq("id", companyId);
  if (error) throw error;
}

/** Ativa um RMA atribuído ao consultor, com validação no backend. */
export async function activateAssignedRma(companyId: string): Promise<void> {
  const { error } = await supabase.functions.invoke("activate-rma", {
    body: { companyId },
  });
 ...
 * Registra automaticamente em `rma_assignment_history`.
 */
