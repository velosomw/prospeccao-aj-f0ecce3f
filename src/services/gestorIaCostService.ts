...
export async function fetchCostIndicators(period: PeriodKey = "mes"): Promise<CostIndicators> {
  ...
  const custoMedioPorProspeccao       = counts.prospecçõesTotal > 0 ? custoTotal / counts.prospecçõesTotal : 0;
  ...
  const custoMedioExecucao     = counts.prospecçõesTotal > 0 ? custoTotal / counts.prospecçõesTotal : 0;
...
export async function fetchPlatformCounts(cutoff: Date | null): Promise<PlatformCounts> {
  ...
  const [prospecçõesTotal, prospecçõesConcluidos, prospecçõesEmAnalise, balancetesRuns, balancetesConsolidados,
  ...
  return {
    prospecçõesTotal, prospecçõesConcluidos, prospecçõesEmAnalise,
