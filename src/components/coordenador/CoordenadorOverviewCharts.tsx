...
const CoordenadorOverviewCharts = ({ companies, assignments, history, profiles }: Props) => {
  // 1. Status dos RMAs (saúde do portfólio)
  ...
  const consultorData = useMemo(() => {
    ...
    const arr = Array.from(counts.entries()).map(([uid, count]) => {
      ...
      return { name, rmas: count };
    });
    return arr.sort((a, b) => b.rmas - a.rmas).slice(0, 6);
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Card className="border-l-4" style={{ borderLeftColor: COLOR_BLUE }}>
          <CardContent className="p-4">
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">{totalAssigned} de {companies.length} RMAs atribuídos</p>
          </CardContent>
        </Card>
        <Card className="border-l-4" style={{ borderLeftColor: COLOR_PURPLE }}>
          <CardContent className="p-4">
            ...
            <p className="text-[11px] text-muted-foreground mt-0.5">RMAs por consultor ativo</p>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Top 6 consultores por nº de RMAs</CardDescription>
          </CardHeader>
          <CardContent className="h-[280px]">
            {consultorData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={consultorData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  <Bar dataKey="rmas" fill={COLOR_PURPLE} radius={[6, 6, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Top 5 setores econômicos por volume de RMAs</CardDescription>
          </CardHeader>
          <CardContent className="h-[280px]">
            {setorData.length === 0 ? (
              <div className="h-full flex items-center justify-center text-sm text-muted-foreground">Sem dados.</div>
