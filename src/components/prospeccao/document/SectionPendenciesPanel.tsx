...
interface Props {
  documentId: string;
  tipo: string; // e.g. "prospeccao_intelligence"
  sections: SectionRow[];
}
...
const SOURCE_HINTS: Record<string, string> = {
  ...
  "prospeccao_cobrancas": "Cobranças/Aging (Workspace › Aging)",
  "prospeccao_analysis_results": "Análise consolidada do Prospeccao",
...
export const SectionPendenciesPanel = ({ documentId, tipo, sections }: Props) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [tpl, evs, srcs] = await Promise.all([
        supabase.from("prospeccao_document_templates").select("structure").eq("tipo", tipo).maybeSingle(),
        supabase.from("prospeccao_section_evidences").select("section_id").eq("document_id", documentId),
        supabase
  ...
  const rows = useMemo(() => {
    return sections.map((s) => {
      ...
      if (evCount === 0) missing.push("Nenhuma evidência ancorada (prospeccao_section_evidences vazia).");
