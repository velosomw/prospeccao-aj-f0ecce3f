...
const generateInsight = (block: DocumentBlock, index: number): AIInsight | null => {
  ...
  const map: Record<AIInsight["status"], { summary: string; detail: string }> = {
    ...
    informativo: {
      ...
      detail: `O tópico "${block.title}" foi formulado com base nas noprospeccaos NBC TA vigentes e dados extraídos via OCR. Os indicadores utilizados seguem os padrões de análise da plataforma.`,
    },
  };
