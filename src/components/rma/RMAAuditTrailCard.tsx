...
export default function RMAAuditTrailCard({ rmaId, companyId }: Props) {
  ...
  useEffect(() => {
    ...
    supabase.from("companies").select("rma_id").eq("id", companyId).maybeSingle()
      .then(({ data }) => { if (data?.rma_id) setResolvedRmaId(data.rma_id); });
  ...
  const load = async () => {
    ...
    try {
      let q = supabase
        ...
        .limit(500);
      if (resolvedRmaId) q = q.eq("rma_id", resolvedRmaId);
    ...
    <Card className="border-2 border-[hsl(217,91%,50%)]/20">
      ...
      <CardContent>
        ...
        <p className="text-[10px] text-muted-foreground mt-2">
          <strong>Parse</strong>: tentativas de leitura via rma-analyze · <strong>Fila</strong>: tentativas no worker assíncrono ·
