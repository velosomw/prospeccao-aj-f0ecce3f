...
// referência (prospeccao_file_parse_cache) e do relatório de referência (lancamentos
...
export default function MonthCoverageCard({ companyId, refreshKey }: Props) {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "prospeccao_file_parse_cache", filter: `company_id=eq.${companyId}` }, () => setTick((t) => t + 1))
      .subscribe();
  ...
  return (
    <div className="mb-4 border border-slate-200 bg-white rounded-lg p-3">
      ...
      <div className="text-[10px] text-muted-foreground mt-2">
        Compara <strong>balancete consolidado</strong> (plataforma) com <strong>prospeccao_file_parse_cache</strong> (referência parsed) e <strong>onedrive_files</strong> em processamento.
