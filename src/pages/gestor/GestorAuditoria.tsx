...
export default function GestorAuditoria() {
  ...
  const load = async () => {
    ...
    let q = supabase
      ...
      .select("id,entity_type,entity_id,action,user_id,user_role,rma_id,conversation_id,summary,created_at")
      .gte("created_at", `${from}T00:00:00`)
      .lte("created_at", `${to}T23:59:59`)
      .order("created_at", { ascending: false })
      .limit(500);
    if (rma.trim()) q = q.ilike("rma_id", `%${rma.trim()}%`);
  ...
  return (
    <ConsultorPageShell
      ...
      kpis={[
        ...
        { label: "Mensagens",        value: stats.msgs,  hint: "RMA & comunicação",icon: FileText,      tone: "purple" },
      ...
      <div className="bg-white rounded-2xl border border-border p-4 mb-4">
        <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
          <div className="md:col-span-1">
            <label className="text-[11px] font-semibold text-muted-foreground">RMA ID</label>
              ...
              return (
                <div key={e.id} className="px-4 py-3 hover:bg-muted/30">
                  <div className="flex items-center flex-wrap gap-2 mb-1">
                    ...
                    {e.rma_id && <span className="text-[10px] font-mono text-purple-700">{e.rma_id}</span>}
