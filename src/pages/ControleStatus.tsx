...
export default function ControleStatus() {
  ...
  useEffect(() => {
    supabase.from("control_users").select("email,name").order("name").then(({ data }) => {
  ...
  useEffect(() => {
    if (!actor) return;
    supabase.from("control_folders").select("*").order("name").then(({ data }) => {
  ...
  const refreshCards = async (fid: string) => {
    const { data } = await supabase.from("control_cards").select("*").eq("folder_id", fid).order("created_at");
  ...
  useEffect(() => {
    ...
    supabase.from("control_card_history").select("*").eq("card_id", openCard.id).order("created_at", { ascending: false })
  ...
  const addFolder = async () => {
    ...
    const { data, error } = await supabase.from("control_folders").insert({ name: newFolderName.trim(), created_by: actor }).select().single();
  ...
  const createCard = async (payload: Partial<Card>) => {
    ...
    const { data, error } = await supabase.from("control_cards").insert({
  ...
  const moveCard = async (card: Card, toStatus: string) => {
    const { error } = await supabase.from("control_cards").update({ status: toStatus }).eq("id", card.id);
    if (error) return toast.error("Erro ao mover");
    await supabase.from("control_card_history").insert({
  ...
  const assignCard = async (card: Card, toResp: string) => {
    const { error } = await supabase.from("control_cards").update({ responsible: toResp }).eq("id", card.id);
  ...
  const updateCard = async (card: Card, patch: Partial<Card>) => {
    const { error } = await supabase.from("control_cards").update(patch).eq("id", card.id);
