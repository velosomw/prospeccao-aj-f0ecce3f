...
import { Plus, Search, Scale, Edit2, Trash2 } from "lucide-react";

// Mock list — substituir por backend (tabela admjudicial_recuperandas / nova tabela)
const mockAJ = [
  { id: 1, razao: "Brasil Expert Administração Judicial", cnpj: "00.000.000/0001-00", responsavel: "Débora Raposo", telefone: "(11) 99999-0000", email: "contato@brasilexpert.com.br" },
];

export default function ConsultorCadastroAJ() {
  const navigate = useNavigate();
  const [search, setSearch] = useState("");
  const rows = mockAJ.filter(r => !search || r.razao.toLowerCase().includes(search.toLowerCase()) || r.cnpj.includes(search));

  return (
    <CadastroPageShell
      breadcrumb={[{ label: "Cadastros", to: "/consultor/cadastro" }, { label: "Administrador Judicial" }]}
      title="Administrador Judicial"
      subtitle="Lista de administradores judiciais cadastrados no sistema."
    >
      <div className="bg-white border border-border rounded-2xl">
        <div className="p-4 flex flex-col sm:flex-row sm:items-center gap-3 border-b border-border">
          ...
          <button
            onClick={() => navigate("/consultor/cadastro/admjudicial/novo")}
        ...
        {rows.length === 0 ? (
          <div className="p-12 text-center">
            <div className="w-14 h-14 rounded-2xl bg-blue-50 flex items-center justify-center mx-auto mb-3">
              <Scale className="w-7 h-7 text-[hsl(217,91%,50%)]" />
            </div>
            <h3 className="text-sm font-semibold">Nenhum administrador cadastrado</h3>
            <p className="text-xs text-muted-foreground mt-1">Clique em "Novo Administrador" para começar.</p>
          </div>
        ) : (
