...
export default function ConsultorRMAs() {
  ...
  const columns = [
    {
      ...
      cell: (r: typeof rows[number]) => (
        <button onClick={() => navigate(`/rma/${r.id}`)} className="font-mono text-primary font-semibold hover:underline">
          {r.id}
        </button>
      ),
    },
    ...
    { key: "dataAtualizacao", header: "Atualização", cell: (r: typeof rows[number]) => <span className="text-muted-foreground text-xs">{r.dataAtualizacao}</span> },
    {
      ...
      cell: (r: typeof rows[number]) => (
        <div className="inline-flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
          <button onClick={() => openRMA(r.id)} className="text-muted-foreground hover:text-foreground"><Eye className="w-4 h-4" /></button>
  ...
  return (
    <ConsultorPageShell
      title="RMAs" subtitle="Todos os Relatórios de Monitoramento sob sua responsabilidade."
      search={search} onSearch={(s) => { setSearch(s); setPage(1); }}
      kpis={[
        { label: "RMAs Totais",  value: stats.total, hint: "No período",      icon: Briefcase,     tone: "blue" },
      ...
      <div className="bg-white rounded-xl border">
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-semibold">Lista de RMAs</h3>
